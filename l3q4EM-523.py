#Lista de Exercício 3 - Questão 4 NÃO FUNCIONOU
#Dupla: 2021327310 - Maria Raissa e 2021314590 - Edson Barros
#Disciplina: Programação Web
#Professor: Italo Arruda

#Supondo que a população de um país A seja da ordem de 80000 habitantes com uma taxa anual de crescimento de 3% e que a população de B seja 200000 habitantes com uma taxa de crescimento de 1.5%. Faça um programa que calcule e escreva o número de anos necessários para que a população do país A ultrapasse ou iguale a população do país B, mantidas as taxas de crescimento.

class Populacao:
    def __init__(self, populacao, taxa_crescimento):
        self.populacao = populacao
        self.taxa_crescimento = taxa_crescimento

    def crescer(self):
        self.populacao += self.populacao * self.taxa_crescimento

    def calcular_anos(self, populacao_alvo):
        anos = 0
        while self.populacao < populacao_alvo:
            self.crescer()
            anos += 1
        return anos


def main():
    try:
        populacao_a = int(input("Informe a população do país A: "))
        taxa_crescimento_a = float(input("Informe a taxa de crescimento do país A (em decimal): "))

        populacao_b = int(input("Informe a população do país B: "))
        taxa_crescimento_b = float(input("Informe a taxa de crescimento do país B (em decimal): "))

        pais_a = Populacao(populacao_a, taxa_crescimento_a)
        pais_b = Populacao(populacao_b, taxa_crescimento_b)

        anos = pais_a.calcular_anos(pais_b.populacao)

        print(f"Serão necessários {anos} anos para a população do país A ultrapassar ou igualar a população do país B.")

    except ValueError:
        print("Erro: Informe valores numéricos válidos para a população e taxa de crescimento.")


if __name__ == '__main__':
    main()

